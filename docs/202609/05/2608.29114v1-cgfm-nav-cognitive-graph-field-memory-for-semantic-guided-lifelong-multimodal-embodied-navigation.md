...
tags: ["query:ml"]
score: 6.0
evidence: 在具身导航中耦合显式多模态关系记忆与空间线索，属跨文本-视觉-空间组合方法
tldr: "视觉语言导航要求智能体在持续探索未知区域时积累并运用观测信息，但现有环境表示难以兼顾显式语义记忆与连续探索引导。为此提出CGFM，以多模态场景图耦合对象、空间关系与视觉观测，支持目标检索与长期推理；匹配不可靠时将图证据投影为语义前沿场，引导探索有语义价值的区域。基于此构建CGFM-Nav，集成子图选择、VLM推理与验证反馈闭环。在GOAT-Bench上，以Qwen3-VL-8B为骨干，成功率由53.2%升至63.0%，SPL由30.0%升至39.6%，验证了显式记忆与语义引导探索结合的有效性。"
source: arxiv
...
motivation: 现有环境表示难以同时支持显式语义记忆与连续探索引导，限制了导航智能体的长期推理与探索决策。
method: 提出CGFM融合多模态场景图与语义前沿场，并构建CGFM-Nav框架，集成子图选择、VLM推理与验证反馈闭环。
result: "在GOAT-Bench上，CGFM-Nav将成功率从53.2%提升至63.0%，SPL从30.0%提升至39.6%。"
conclusion: 显式语义记忆与语义引导探索结合可显著提升持续多模态导航的性能与鲁棒性。
---

## 摘要
视觉-语言导航（VLN）要求智能体在持续探索未知区域的同时，对累积的观察进行推理。然而，现有的环境表示往往难以同时支持显式语义记忆和连续探索引导。为解决这一挑战，我们提出了认知图-场记忆（CGFM），一种将显式关系记忆与连续空间直觉耦合的持久化多模态场景表示。CGFM 将对象、空间关系和视觉观察组织成多模态场景图，从而支持跨导航任务的目标检索和长时程推理。当无法识别到可靠的目标匹配时，基于图的证据会被投影到一个由目标条件化的语义前沿场中，以引导探索朝向语义上有前途的前沿和区域。基于 CGFM，我们引入了 CGFM-Nav，一个基于基础模型的终身多模态导航框架，它将任务相关子图选择、VLM 推理和验证反馈整合到一个闭环决策循环中。在 GOAT-Bench 上的初步实验表明，在相同的 Qwen3-VL-8B 骨干网络下，CGFM-Nav 将整体成功率从 53.2% 提升至 63.0%，SPL 从 30.0% 提升至 39.6%，证明了将显式语义记忆与语义引导探索相结合的有效性。
